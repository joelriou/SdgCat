...
When `D` is a Cartesian monoidal category, ring objects in `C ⥤ D` are the same
thing as functors from `C` into the ring objects of `D`.

This is formalised as:
* `rngFunctorCategoryEquivalence : RingObjCat (C ⥤ D) ≌ C ⥤ RingObjCat D`
-/
...
namespace RngFunctorCategoryEquivalence
...
/-- A ring object in a functor category sends any object to a ring object. -/
@[simps]
def functorObjObj (A : C ⥤ D) [RingObj A] (X : C) : RingObjCat D where
  X := A.obj X
  ringObj :=
  { zero := ζ[A].app X
    add := σ[A].app X
...
/-- A ring object in a functor category induces a functor to the category of ring objects. -/
@[simps]
def functorObj (A : C ⥤ D) [RingObj A] : C ⥤ RingObjCat D where
  obj := functorObjObj A
  map f :=
    { hom := A.map f
      isRingHom :=
...
/-- Functor translating a ring object in a functor category to a functor into ring objects. -/
@[simps]
def functor : RingObjCat (C ⥤ D) ⥤ C ⥤ RingObjCat D where
...
/-- A functor to the category of ring objects can be translated as a ring object in the functor
category. -/
@[simps]
def inverseObj (F : C ⥤ RingObjCat D) : RingObjCat (C ⥤ D) where
  X := F ⋙ RingObjCat.forget D
  ringObj :=
...
/-- Functor translating a functor into the category of ring objects to a ring object in the functor
category. -/
@[simps]
def inverse : (C ⥤ RingObjCat D) ⥤ RingObjCat (C ⥤ D) where
  obj := inverseObj
  map α :=
    { hom :=
        { app := fun X => (α.app X).hom
          naturality := fun _ _ f => congr_arg RingObjCat.Hom.hom (α.naturality f) }
      isRingHom :=
...
/-- The unit for the equivalence `RingObjCat (C ⥤ D) ≌ C ⥤ RingObjCat D`. -/
@[simps!]
def unitIso : 𝟭 (RingObjCat (C ⥤ D)) ≅ functor ⋙ inverse :=
  NatIso.ofComponents (fun A =>
  { hom := { hom := 𝟙 _, isRingHom := { } }
    inv := { hom := 𝟙 _, isRingHom := { } } })

/-- The counit for the equivalence `RingObjCat (C ⥤ D) ≌ C ⥤ RingObjCat D`. -/
@[simps!]
def counitIso : inverse ⋙ functor ≅ 𝟭 (C ⥤ RingObjCat D) :=
  NatIso.ofComponents (fun A =>
    NatIso.ofComponents (fun X =>
      { hom := { hom := 𝟙 _, isRingHom := { } }
        inv := { hom := 𝟙 _, isRingHom := { } } }))

end RngFunctorCategoryEquivalence

open RngFunctorCategoryEquivalence

/-- When `D` is a Cartesian monoidal category, ring objects in `C ⥤ D` are the same thing as
functors from `C` into the ring objects of `D`. -/
@[simps]
def rngFunctorCategoryEquivalence : RingObjCat (C ⥤ D) ≌ C ⥤ RingObjCat D where
  functor := functor
  inverse := inverse
  unitIso := unitIso
  counitIso := counitIso
